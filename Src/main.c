...
static void Deinterleave_Pair(uint32_t off, uint8_t pair)
{
   ...
   * CAVEAT: SAI2 was once suspected of a broken bit23 on the OLD mics; re-verify the
   * SAI2 channels (ch4..7) read clean 24-bit with the new mics via a RAW1 capture
   * (scripts/diag_mic_channels.ps1 or tools/read_mic_raw.m). */
  uint8_t wide = 1U;
